...
class UserVoice(Base):
    ...
    def __repr__(self) -> str:
        return (
            f'UserVoice(guild_id={self.guild_id}, user_id={self.user_id}, '
        ...
        if voice is None:
            result = await session.execute(
                ...
                .values(
                    ...
                    user_id=member.id,
                    voice=config[member.guild].default_voice,
                )
                .returning(UserVoice)
            )
...
async def update_voice(
    ...
    *,
    ...
    if (new_voice, new_pitch, new_speed) == (
        current.voice,
        current.pitch,
        current.speed,
    ):
    ...
    async with async_session() as session:
        updated = await session.execute(
            update(UserVoice)
            .filter_by(guild_id=member.guild.id, user_id=member.id)
