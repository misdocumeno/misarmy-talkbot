...
from misarmy_talkbot.infra.config.config import config
...
class VoicePreset(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            'guild_id', 'user_id', 'name', name='uix_guild_user_name'
        ),
    )

    def __repr__(self) -> str:
        return (
            f'VoicePreset(guild_id={self.guild_id}, user_id={self.user_id}, '
            f'name={self.name!r}, voice={self.voice!r}, pitch={self.pitch}, speed={self.speed})'
        )


_user_presets: dict[
    discord.Guild, dict[discord.Member, list[VoicePreset]]
] = {}
...
async def get_presets(member: discord.Member) -> list[VoicePreset]:
    ...
    if member.guild in _user_presets and member in _user_presets[member.guild]:
        return _user_presets[member.guild][member] + get_guild_presets(
            member.guild
        )
...
async def save_preset(
    member: discord.Member, name: str, voice: str, pitch: float, speed: float
) -> bool:
