...
from ..utils import reply_interaction, reply_unsupported_locale
from .config import default_config, get_config_json, update_config
from ..locale.translations import translate, UnsupportedLocaleError
...
async def on_config_mention(message: discord.Message) -> bool:
    ...
    """
    assert message.guild is not None and isinstance(message.author, discord.Member)
    ...
    if match:
        json = match.group(1)
    elif attachment and attachment.content_type and attachment.content_type.endswith('charset=utf-8'):
